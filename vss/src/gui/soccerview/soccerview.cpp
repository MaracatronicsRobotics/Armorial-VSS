...
void SoccerView::updateDetection(VSSTeam *ourTeam, VSSTeam *theirTeam) {
    graphicsMutex.lock();
    ...
    QList<VSSPlayer *> ourPlayers = ourTeam->avPlayers().values();
    for(int x = 0; x < ourPlayers.size(); x++){
        robot.id = ourPlayers.at(x)->playerId();
        robot.hasAngle = true;
        robot.team = (ourPlayers.at(x)->playerTeam()->teamColor() == Colors::BLUE ? TeamTypes::teamBlue : TeamTypes::teamYellow);
    ...
    for(int x = 0; x < theirPlayers.size(); x++){
        robot.id = theirPlayers.at(x)->playerId();
        robot.hasAngle = true;
        robot.team = (theirPlayers.at(x)->playerTeam()->teamColor() == Colors::BLUE ? TeamTypes::teamBlue : TeamTypes::teamYellow);
        robot.angle = theirPlayers.at(x)->angle().value() * 180.0/M_PI;
        robot.loc.set(theirPlayers.at(x)->position().x() * 1000.0, theirPlayers.at(x)->position().y() * 1000.0);
        robots.append(robot);
    }
