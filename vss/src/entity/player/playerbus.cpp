...
VSSTeam* PlayerBus::_ourTeam = NULL;
VSSTeam* PlayerBus::_theirTeam = NULL;

void PlayerBus::initialize(VSSTeam *ourTeam, VSSTeam *opTeam) {
    PlayerBus::_ourTeam = ourTeam;
    PlayerBus::_theirTeam = opTeam;
}

bool PlayerBus::ourPlayerAvailable(quint8 id) {
    QHash<quint8,VSSPlayer*> avPlayers = PlayerBus::_ourTeam->avPlayers();
    return avPlayers.contains(id);
}

bool PlayerBus::theirPlayerAvailable(quint8 id) {
    QHash<quint8,VSSPlayer*> avOpPlayers = PlayerBus::_theirTeam->avPlayers();
