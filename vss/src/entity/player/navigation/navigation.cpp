...
Navigation::Navigation(VSSPlayer *player, NavigationAlgorithm *navAlg, VSSConstants *mrcconstants) {
    _player = player;
    _navAlg = navAlg;
...
void Navigation::setGoal(const Position &destination, const Angle &orientation, bool avoidTeammates, bool avoidOpponents, bool avoidBall, bool avoidOurGoalArea, bool avoidTheirGoalArea) {
    ...
    if(avoidTeammates) {
        QList<VSSPlayer*> avPlayers = _ourTeam->avPlayers().values();
        for(int i=0; i<avPlayers.size(); i++) {
            VSSPlayer *teammate = avPlayers.at(i);
    ...
    if(avoidOpponents) {
        QList<VSSPlayer*> avOpPlayers = _opTeam->avPlayers().values();
        for(int i=0; i<avOpPlayers.size(); i++) {
            VSSPlayer *opponent = avOpPlayers.at(i);
